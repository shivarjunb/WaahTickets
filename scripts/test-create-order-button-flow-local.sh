...
if [[ "$payments_count" -ne 0 || "$tickets_count" -eq 0 ]]; then
  cat <<TROUBLESHOOT >&2

Button flow mismatch: payments rows should remain zero, and tickets should be generated by queue processing.
